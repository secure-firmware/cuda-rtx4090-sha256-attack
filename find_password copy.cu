...
__global__ void find_password(long long start, long long end, const char *target_password, int *found, long long *result_index, const char *target_salt, const uint8_t *target_hash, uint8_t *result_hash)
{
    long long idx = blockIdx.x * blockDim.x + threadIdx.x + start;
    uint8_t sha256_uint8[32];
    char result_hash_local[hash_length + 1];
